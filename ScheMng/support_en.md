...
- If a reminder never appears, check **iOS Settings → Notifications → Schedule Manager** and make sure Allow Notifications, Banners, Sounds and Badges are all on.
...
- Long-press an empty area of the home screen, tap **+** at the top left and search for `Schedule Manager` (or `자녀일정`).
...
- Schedule Manager 1.0 is **free with a Google AdMob banner**. A single 50pt banner appears at the bottom of the main screen.
...
- Deleting Schedule Manager from iPhone removes all on-device data.
